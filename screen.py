...
def res(jobfile):
    ...
    for i in Resume_Vector:
        if i==[]:
            Ordered_list_Resume_Score.append(2)
            continue
        samples = i
    ...
    print(len(Ordered_list_Resume), len(Ordered_list_Resume_Score), len(Resumes))
   # plt.plot(range(len(Ordered_list_Resume)),Ordered_list_Resume_Score)
   # plt.xlabel("Resumes")
   # plt.ylabel("Scores")
    #plt.show()
    
   # plt.pie(Ordered_list_Resume_Score, labels = Ordered_list_Resume)
   # plt.show()
